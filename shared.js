...
const CAT = {
  dine: { id:'dine', label:'Dine In', prefix:'', color:'#C8502A', light:'#FDF0EB', light2:'#FFF8F5' },
};
...
let state = {
  ...
  announcements:  [],
  activeCounters: 1,
  waSettings:     { sendOnIssue: true, sendOnCall: true },
};
...
function sendWANotification(ticket, type) {
  ...
  const qLen   = state.queues[ticket.category] ? state.queues[ticket.category].length : 0;
  const wMins  = estimateWait(qLen - 1, state.activeCounters, state.waitSettings[ticket.category] || 12);
  const wText  = wMins ? ' Estimated wait: *~' + wMins + ' minutes*.' : '';
  const messages = {
    issued:   'Hello! 👋 You have been assigned queue number *' + ticket.number + '* at *' + state.restaurantName + '*.' + wText + ' We will notify you when your turn is near. Thank you! 🍽',
...
async function issueTicket(catId, phone, pax) {
  ...
  const num    = String(state.ticketNums[catId]).padStart(3, '0');
  ...
  await saveState();
  if (typeof render === 'function') render();
  // Send WA confirmation to customer on ticket issue
  if (ticket.phone && state.waSettings && state.waSettings.sendOnIssue) {
    const link = sendWANotification(ticket, 'issued');
...
async function callSpecific(ticketId, catId) {
  ...
  if (next.phone && state.waSettings && state.waSettings.sendOnCall) {
    const link = sendWANotification(next, 'called');
    if (link) window.open(link, '_blank', 'noopener');
  }
  if (nextUp && nextUp.phone && state.waSettings && state.waSettings.sendOnCall) sendWANotification(nextUp, 'upcoming');
...
async function saveWASettings(sendOnIssue, sendOnCall) {
  if (!state.waSettings) state.waSettings = {};
  state.waSettings.sendOnIssue = sendOnIssue;
  state.waSettings.sendOnCall  = sendOnCall;
  await saveState();
